...
public:
    ...
    cItem(int L, int W, int H, int D)
        : cSpace(L, W, H), demand(D), myPacked(false)
    {
    }
...
void sInstance::read(const std::string &fname)
{
    ...
    std::cout << "length: "<< bin[0] << " " << "width: "<<  bin[1] << " " <<"height: "<<  bin[2] << " " << "\n";
}
    ...
    sInstance &problem)
{
    ...
    In the Queiroz paper reduced raster points are used as an optimization.
